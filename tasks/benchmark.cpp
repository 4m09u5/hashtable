//
// Created by dmitr on 04.05.2023.
//
...
int getPerformanceTime(void ownFunc(struct ListNode* list, struct HashTable*), void builtInFunc(struct ListNode* list,  std::unordered_map<char*, char*> map), struct ListNode* list, struct HashTable* table,  std::unordered_map<char*, char*> map){
    clock_t start, end;
    start = clock();
    ownFunc(list, table);
    end = clock();
